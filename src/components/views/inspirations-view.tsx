...
const STATUS_CONFIG: Record<string, { label: string; color: string; bg: string; icon: React.ElementType }> = {
  pending: { label: '待分析', color: 'text-amber-600', bg: 'bg-amber-500/10 border-amber-500/30', icon: Clock },
  analyzing: { label: '分析中', color: 'text-blue-600', bg: 'bg-blue-500/10 border-blue-500/30', icon: Loader2 },
  converted: { label: '已转化', color: 'text-emerald-600', bg: 'bg-emerald-500/10 border-emerald-500/30', icon: CheckCircle2 },
  dismissed: { label: '已忽略', color: 'text-gray-500', bg: 'bg-gray-500/10 border-gray-500/30', icon: XCircle },
}

const SOURCE_CONFIG: Record<string, { label: string; icon: React.ElementType; color: string }> = {
  chat: { label: '聊天', icon: MessageSquare, color: 'text-blue-500' },
  voice: { label: '语音', icon: Mic, color: 'text-purple-500' },
      ...
      <Card className="hover:shadow-md transition-all duration-200 hover:-translate-y-0.5 group">
        <CardContent className="p-4 space-y-3">
          ...
          <div className="flex items-start gap-3">
            <div className={`flex size-9 items-center justify-center rounded-lg shrink-0 ${statusConfig.bg} border`}>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mt-2">
                ...
                {/* Status badge */}
                <Badge variant="outline" className={`text-[10px] px-1.5 py-0 ${statusConfig.bg} ${statusConfig.color} border`}>
                  {statusConfig.label}
                </Badge>
                {/* Time */}
                ...
                <div key={issue.id} className="flex items-center gap-2 text-xs">
                  ...
                  <Badge
                    ...
                    className={`text-[9px] px-1 py-0 shrink-0 ${
                      ...
                      issue.status === 'in_progress' ? 'border-blue-500/30 text-blue-600' :
  ...
  return (
    ...
    >
      ...
      <motion.div variants={item} className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <Button className="gap-2 hover:-translate-y-0.5 transition-all" onClick={() => setShowInspirationInput(true)}>
        ...
        <motion.div variants={item}>
          <Card>
            <CardContent className="py-16 flex flex-col items-center justify-center text-muted-foreground">
              <Lightbulb className="size-12 mb-4 opacity-30" />
              <p className="text-lg font-medium">暂无灵感记录</p>
